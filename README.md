...
* 请你将ysyx-workbench上传到一个公开的仓库，仓库增加doc文件夹，上传nemu和npc运行PAL通过的截图以及time.txt, 截图名：ysyx学号-nemu, ysyx学号-npc, time.txt标明nemu和npc运行PAL出现logo SOFTSTAR所需时间，精确到分，如果所需时间过长请自行优化以提升通过考核的机会。
...
1. 助教每周安排线上调试考核（每人45分钟），每两名助教监考一位学生（注入bug与监考人员必须不一致）
